...
public class Cine {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		do{
			...
			case 11: //Comprar ENTRADA
				...
				System.out.println("Pel·Licules en cartelera:");
				// 1 mostrar llistat pel·licules disponibles
				if (cine.llistarPelicules() == 0) {
					System.out.println("\tNo hi ha pel·lícules disponibles");
					return;
				}
				AccesTaquilla accesTaquilla = new AccesTaquilla();
				Thread filOne = new Thread(new FilCompraEntrada(cine, accesTaquilla),"Fil uno");
				Thread filTwo = new Thread(new FilCompraEntrada(cine, accesTaquilla),"Fil dos");
				...
				//cine.compraEntradaPelicula(cine);
				break;
				//********
	...
	public void compraEntradaPelicula(){
		...
		// 2.- Selecció de la pel.licula 
		Pelicula pelicula;
		//System.out.println("[" +Thread.currentThread().getName()+"] Comprar una entrada...");
		int numPelicula;
		numPelicula = Validacio.validaSencer("Pel·licula: ", pelicules.size());
		pelicula = pelicules.get(numPelicula-1);
		System.out.println("\t[" +Thread.currentThread().getName()+ "]Pel·licula escollida:\n"  +pelicula);
		
		// 3.- Selecció de la sessió
		Sessio sessio;
		Sala sala;
		int numSessio;
		System.out.println("Sessions per aquesta pel·licula");
		if (pelicula.llistarSessionsPeli() == 0) {
			System.out.println("\tNo hi ha sessió disponible per " +pelicula.getNomPeli());
			return;
		}
		
		numSessio = Validacio.validaSencer("Sessió: ", pelicula.getSessionsPeli().size());
		sessio = pelicula.retornaSessioPeli(numSessio);
			// obtenim la sala
		sala = sessio.getSala();
		sessio.mapaSessio();
		// 5.- L'usuari introduirà quantes entrades es vol comprar.
		//int numEntrades = Validacio.validaSencer("Número d'entrades a comprar:", numSessio);
		
		//5.- Seleccionar el seient a reservar, selecció de la fila i del seient, el qual quedarà reservat.
		Seient [][] platea = sessio.getSeients();
		int fila, seient;
		
		fila = Validacio.validaSencer("Fila: " /*+sala.getFiles()*/, sala.getFiles());
		seient = Validacio.validaSencer("Seient: " /*+sala.getTamanyFila()*/, sala.getTamanyFila());
		
		//reserva de la localitat
		if (platea[fila-1][seient-1].verificaSeient()) {
			platea[fila-1][seient-1].reservaSeient();
			System.out.println("Localitat reservada:\n\t-Fila: " +fila+ " -Seient: " +seient);
			
			// 6. Pagament de l’entrada i compra de l'entrada del seient seleccionat.
			if (pagamentEntrada(sessio.getPreu())) {
				System.out.println("\nOperació acceptada. Localitat ocupada:\n\t-Fila: " +fila+ " -Seient: " +seient);
				
				//7. Si s’ha pagat exitosament, el Seient reservat passarà a l’estat ocupat
				platea[fila-1][seient-1].ocupaSeient();
				// imprimir ticket de l’entrada.
				
				sessio.imprimirTicket(platea[fila-1][seient-1], sessio, sala, pelicula);
				
			} else {
				// 8. Si no s’ha pogut fer el pagament, el Seient reservat passarà a  lliure i disponible per a qualsevol client.
				System.out.println("Operació cancelada.");
				platea[fila-1][seient-1].alliberaSeient();
			}
			
		} else {
			System.out.println("No s'ha pogut reservar la localitat seleccionada.");
		}
	// 9. Mapa resultant de la selecció de Seients amb elsseients lliures, reservats i ocupats	
	sessio.mapaSessio();
	...
	boolean pagamentEntrada(BigDecimal preu){
		...
		// 2. Simular el pagament (en targeta o en metàl·lic, dades de targeta si es cal, confirmació de compra,..) 
		//introduint un retard explicit típic del procés de pagament (en aquest cas, 3 segons).
		try {
			Thread.sleep(3000);
		} catch (InterruptedException e) {
		...
		// 3. Confirmació del pagament i es retornarà si s’ha fet el pagament amb èxit o no.
		return Validacio.validaBoolea("Confirmar pagament ==> S / N");
	}//pagamentEntrada
